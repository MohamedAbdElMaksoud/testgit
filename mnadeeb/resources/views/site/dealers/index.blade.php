        ...
        <div class="page-content">
            <div class="portlet light portlet-fit bordered">
                <div class="portlet-title">
                    ...
                    <div class="actions">
                        <div class="btn-group">
                            <a class="btn dark btn-outline btn-circle btn-sm" href="{{url('dealers/create')}}">
                                اضافة
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="table-scrollable">
                        <table class="table table-bordered table-hover">
                            ...
                            @foreach($items as $item)
                                <tr>
                                    <td>{{$item->id}}</td>
                                    <td><img src="{{$item->dealer_id}}" width="50" height="50"></td>
                                    ...
                                    <td>{{$item->minmum}}</td>
                                    <td>{{$item->maxmum}}</td>
                                    <td>{{$item->delivery_time_min}}</td>
                                    <td>{{$item->delivery_time_max}}</td>

                                    <td>
                                        <a href="{{url("dealers/$item->id/edit")}}">
                                            <button class="btn btn-warning label-sm">تعديل</button>
                                        </a>
                                        <form onsubmit='return ConfirmDelete()' method="post"
                                              action="{{url("dealers/$item->id")}}">
                                            {{ method_field('DELETE') }}
